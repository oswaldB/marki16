// backend/cloud/relances/jobs/import-invoices/01-syncImpayes.js
// Synchronise les impayés et leurs contacts depuis la DB SQLite externe (Analyse immo).
...
// Requête 1 : Pièces + Dossiers (sans interlocuteurs)
let QUERY_PIECES = `
  SELECT
    p.idpiece,
    p.nfacture,
...
async function syncImpayes({ trigger = 'cron' } = {}) {
  ...
  info('Début de la synchronisation des impayés', 'import-invoice', 'syncImpayes', { trigger });
  debug('Initialisation des statistiques', 'import-invoice', 'syncImpayes', { stats });
  ...
  debug(`Chemin DB: ${dbPath}`, 'import-invoice', 'syncImpayes', { dbPath });
  ...
  async function openDatabaseWithRetry(path, maxRetries = 3, retryDelayMs = 60000) {
    ...
    while (retries < maxRetries) {
      try {
        ...
        info(`Connexion DB SQLite réussie (${path})`, 'import-invoice', 'syncImpayes', { path, attempt: retries + 1 });
        return db;
      } catch (err) {
        retries++;
        error(`Erreur DB SQLite (attempt ${retries}/${maxRetries}): ${err.message}`, 'import-invoice', 'syncImpayes', { error: err.message, attempt: retries, maxRetries });
        if (err.message.includes('database disk image is malformed') && retries < maxRetries) {
          warn(`Attente de 1 minute avant retry...`, 'import-invoice', 'syncImpayes', { retryDelayMs });
  ...
  debug('Ouverture de la base de données', 'import-invoice', 'syncImpayes');
  const db = await openDatabaseWithRetry(dbPath);
  
  try {
    debug('Étape 1: Récupération des pièces + dossiers', 'import-invoice', 'syncImpayes');
    // Étape 1 : Récupérer toutes les pièces + dossiers
    const piecesRows = db.prepare(QUERY_PIECES).all();
    info(`${piecesRows.length} pièces récupérées depuis la base externe`, 'import-invoice', 'syncImpayes', { count: piecesRows.length });
    
    debug('Étape 2: Récupération des statuts', 'import-invoice', 'syncImpayes');
    ...
    try {
      ...
      debug(`Statuts chargés: ${Object.keys(statutsMap).length}`, 'import-invoice', 'syncImpayes', { statutsCount: Object.keys(statutsMap).length });
    } catch(e) { 
      warn(`Erreur statuts: ${e.message}`, 'import-invoice', 'syncImpayes', { error: e.message }); 
    }
    
    debug('Étape 3: Récupération des employés', 'import-invoice', 'syncImpayes');
    ...
    try {
      ...
      debug(`Employés chargés: ${Object.keys(employesMap).length}`, 'import-invoice', 'syncImpayes', { employesCount: Object.keys(employesMap).length });
    } catch(e) { 
      warn(`Erreur employés: ${e.message}`, 'import-invoice', 'syncImpayes', { error: e.message }); 
    }
    
    debug('Étape 4: Récupération des interlocuteurs par dossier', 'import-invoice', 'syncImpayes');
    // Étape 4 : Récupérer tous les interlocuteurs par dossier (en une seule requête)
    const dossierIds = [...new Set(piecesRows.map(r => r.dossier_id).filter(id => id != null))];
    ...
    try {
      ...
      info(`Interlocuteurs chargés: ${Object.keys(interlocuteursByDossier).length} dossiers`, 'import-invoice', 'syncImpayes', { dossiersCount: Object.keys(interlocuteursByDossier).length });
    } catch(e) { 
      warn(`Erreur interlocuteurs: ${e.message}`, 'import-invoice', 'syncImpayes', { error: e.message }); 
    }

    info('Début du traitement des impayés', 'import-invoice', 'syncImpayes');

    for (const pieceRow of piecesRows) {
      try {
        debug(`Traitement de l'impayé nfacture=${pieceRow.nfacture}`, 'import-invoice', 'syncImpayes', { nfacture: pieceRow.nfacture });
        ...
        // Sauvegarde
        debug(`Sauvegarde de l'impayé nfacture=${pieceRow.nfacture}`, 'import-invoice', 'syncImpayes', { nfacture: pieceRow.nfacture, isNew: isNewImpaye });
        await impaye.save(null, { useMasterKey: true });
        info(`Impayé sauvegardé avec succès (nfacture=${pieceRow.nfacture})`, 'import-invoice', 'syncImpayes', { nfacture: pieceRow.nfacture, isNew: isNewImpaye });
        ...
        try {
          ...
          error(`Erreur log activite pour ${pieceRow.nfacture}: ${logErr.message}`, 'import-invoice', 'syncImpayes', { nfacture: pieceRow.nfacture, error: logErr.message });
        }

      } catch (err) {
        error(`Erreur nfacture=${pieceRow.nfacture}: ${err.message}`, 'import-invoice', 'syncImpayes', { nfacture: pieceRow.nfacture, error: err.message, stack: err.stack?.substring(0, 500) });
        debug(`Stack trace: ${err.stack?.substring(0, 1000)}`, 'import-invoice', 'syncImpayes', { nfacture: pieceRow.nfacture });
        ...
        try {
          ...
          error(`Erreur log activite erreur pour ${pieceRow.nfacture}: ${logErr.message}`, 'import-invoice', 'syncImpayes', { nfacture: pieceRow.nfacture, error: logErr.message });
        }
      }
    }

    info(`Terminé — ${stats.impayes_created} créés, ${stats.impayes_updated} MàJ, ${stats.contacts_created} contacts créés, ${stats.errors.length} erreurs`, 'import-invoice', 'syncImpayes', { 
    ...
    error(`Erreur base de données SQLite: ${err.message}`, 'import-invoice', 'syncImpayes', { error: err.message, stack: err.stack?.substring(0, 500) });
    stats.errors.push({ error: err.message });
  } finally {
    db.close();

    // Persistance du log d'exécution dans Parse
    debug('Écriture du log dans Parse (SyncLog)', 'import-invoice', 'syncImpayes');
    try {
      ...
      info('Log Parse sauvegardé avec succès', 'import-invoice', 'syncImpayes');
    } catch (logErr) {
      error(`Impossible d'écrire le SyncLog: ${logErr.message}`, 'import-invoice', 'syncImpayes', { error: logErr.message, stack: logErr.stack?.substring(0, 500) });
    }
  }
