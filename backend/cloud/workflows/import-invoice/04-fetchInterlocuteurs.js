...
// Retourne : { interlocuteursByDossier: {}, state: {} }
 ...
 * @param {Object} param0 - { pieces: [], state: {} }
 * @returns {Promise<Object>} { interlocuteursByDossier: {}, state: {} }
 */
async function fetchInterlocuteurs({ pieces, state }) {
  ...
  try {
    info('Étape 4: Récupération des interlocuteurs par dossier', 'import-invoice', 'fetchInterlocuteurs');
    
    // Extraire les dossierIds uniques des pièces
    const dossierIds = [...new Set(pieces
    ...
    info(`Interlocuteurs chargés: ${Object.keys(interlocuteursByDossier).length} dossiers`, 'import-invoice', 'fetchInterlocuteurs', { dossiersCount: Object.keys(interlocuteursByDossier).length });

    const newState = {
      ...state,
      currentStep: '05-processAndSaveImpayes',
      steps: {
        ...state.steps,
        '04-fetchInterlocuteurs': {
          status: 'completed',
          dossierCount: Object.keys(interlocuteursByDossier).length,
    ...
    error(`Erreur Étape 4: ${err.message}`, 'import-invoice', 'fetchInterlocuteurs', { error: err.message });
    throw err;
  } finally {
    db.close();
  }
