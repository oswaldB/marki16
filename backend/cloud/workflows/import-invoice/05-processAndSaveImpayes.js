 ...
 */
async function processAndSaveImpayes({ pieces, statutsMap, employesMap, interlocuteursByDossier, state }) {
  const stats = { 
    impayes_created: 0, 
    impayes_updated: 0, 
    contacts_created: 0, 
  ...
  info('Étape 5: Début du traitement des impayés', 'import-invoice', 'processAndSaveImpayes', { pieceCount: pieces.length });

  try {
    for (const pieceRow of pieces) {
      try {
        debug(`Traitement de l'impayé nfacture=${pieceRow.nfacture}`, 'import-invoice', 'processAndSaveImpayes', { nfacture: pieceRow.nfacture });
        ...
        impaye.set('payeur_nom', getInterlocuteurField(interlocuteurs, 'Payeur', 'nom') || null);
        impaye.set('payeur_prenom', getInterlocuteurField(interlocuteurs, 'Payeur', 'prenom') || null);
        impaye.set('payeur_email', getInterlocuteurField(interlocuteurs, 'Payeur', 'email') || null);
        impaye.set('payeur_telephone', getInterlocuteurField(interlocuteurs, 'Payeur', 'telephone') || null);
        impaye.set('payeur_type_personne', getInterlocuteurField(interlocuteurs, 'Payeur', 'typePersonne') || null);
        impaye.set('payeur_contact_nom', getInterlocuteurContactField(interlocuteurs, 'Payeur', 'contact_nom') || null);
        impaye.set('payeur_contact_prenom', getInterlocuteurContactField(interlocuteurs, 'Payeur', 'contact_prenom') || null);
        impaye.set('payeur_contact_email', getInterlocuteurContactField(interlocuteurs, 'Payeur', 'contact_email') || null);

        impaye.set('apporteur_nom', getInterlocuteurField(interlocuteurs, "Apporteur d'affaire", 'nom') || null);
        impaye.set('apporteur_prenom', getInterlocuteurField(interlocuteurs, "Apporteur d'affaire", 'prenom') || null);
        impaye.set('apporteur_email', getInterlocuteurField(interlocuteurs, "Apporteur d'affaire", 'email') || null);
        impaye.set('apporteur_telephone', getInterlocuteurField(interlocuteurs, "Apporteur d'affaire", 'telephone') || null);
        impaye.set('apporteur_contact_nom', getInterlocuteurContactField(interlocuteurs, "Apporteur d'affaire", 'contact_nom') || null);
        impaye.set('apporteur_contact_prenom', getInterlocuteurContactField(interlocuteurs, "Apporteur d'affaire", 'contact_prenom') || null);
        impaye.set('apporteur_contact_email', getInterlocuteurContactField(interlocuteurs, "Apporteur d'affaire", 'contact_email') || null);

        impaye.set('acquereur_nom', getInterlocuteurField(interlocuteurs, 'Acquéreur', 'nom') || null);
        impaye.set('acquereur_prenom', getInterlocuteurField(interlocuteurs, 'Acquéreur', 'prenom') || null);
        impaye.set('acquereur_email', getInterlocuteurField(interlocuteurs, 'Acquéreur', 'email') || null);
        impaye.set('acquereur_telephone', getInterlocuteurField(interlocuteurs, 'Acquéreur', 'telephone') || null);

        impaye.set('donneur_ordre_nom', getInterlocuteurField(interlocuteurs, 'Donneur d\'ordre', 'nom') || null);
        impaye.set('donneur_ordre_prenom', getInterlocuteurField(interlocuteurs, 'Donneur d\'ordre', 'prenom') || null);
        impaye.set('donneur_ordre_email', getInterlocuteurField(interlocuteurs, 'Donneur d\'ordre', 'email') || null);
        impaye.set('donneur_ordre_telephone', getInterlocuteurField(interlocuteurs, 'Donneur d\'ordre', 'telephone') || null);

        impaye.set('locataire_entrant_nom', getInterlocuteurField(interlocuteurs, 'Locataire entrant', 'nom') || null);
        impaye.set('locataire_entrant_prenom', getInterlocuteurField(interlocuteurs, 'Locataire entrant', 'prenom') || null);
        impaye.set('locataire_entrant_email', getInterlocuteurField(interlocuteurs, 'Locataire entrant', 'email') || null);
        impaye.set('locataire_entrant_telephone', getInterlocuteurField(interlocuteurs, 'Locataire entrant', 'telephone') || null);

        impaye.set('locataire_sortant_nom', getInterlocuteurField(interlocuteurs, 'Locataire sortant', 'nom') || null);
        impaye.set('locataire_sortant_prenom', getInterlocuteurField(interlocuteurs, 'Locataire sortant', 'prenom') || null);
        impaye.set('locataire_sortant_email', getInterlocuteurField(interlocuteurs, 'Locataire sortant', 'email') || null);
        impaye.set('locataire_sortant_telephone', getInterlocuteurField(interlocuteurs, 'Locataire sortant', 'telephone') || null);

        impaye.set('notaire_nom', getInterlocuteurField(interlocuteurs, 'Notaire', 'nom') || null);
        impaye.set('notaire_prenom', getInterlocuteurField(interlocuteurs, 'Notaire', 'prenom') || null);
        impaye.set('notaire_email', getInterlocuteurField(interlocuteurs, 'Notaire', 'email') || null);
        impaye.set('notaire_telephone', getInterlocuteurField(interlocuteurs, 'Notaire', 'telephone') || null);

        impaye.set('proprietaire_nom', getInterlocuteurField(interlocuteurs, 'Propriétaire', 'nom') || null);
        impaye.set('proprietaire_prenom', getInterlocuteurField(interlocuteurs, 'Propriétaire', 'prenom') || null);
        impaye.set('proprietaire_email', getInterlocuteurField(interlocuteurs, 'Propriétaire', 'email') || null);
        impaye.set('proprietaire_telephone', getInterlocuteurField(interlocuteurs, 'Propriétaire', 'telephone') || null);
        impaye.set('proprietaire_type_personne', getInterlocuteurField(interlocuteurs, 'Propriétaire', 'typePersonne') || null);
        impaye.set('proprietaire_contact_nom', getInterlocuteurContactField(interlocuteurs, 'Propriétaire', 'contact_nom') || null);
        impaye.set('proprietaire_contact_prenom', getInterlocuteurContactField(interlocuteurs, 'Propriétaire', 'contact_prenom') || null);
        impaye.set('proprietaire_contact_email', getInterlocuteurContactField(interlocuteurs, 'Propriétaire', 'contact_email') || null);

        impaye.set('syndic_nom', getInterlocuteurField(interlocuteurs, 'Syndic', 'nom') || null);
        impaye.set('syndic_prenom', getInterlocuteurField(interlocuteurs, 'Syndic', 'prenom') || null);
        impaye.set('syndic_email', getInterlocuteurField(interlocuteurs, 'Syndic', 'email') || null);
        impaye.set('syndic_telephone', getInterlocuteurField(interlocuteurs, 'Syndic', 'telephone') || null);
        ...
        // Sauvegarde
        debug(`Sauvegarde de l'impayé nfacture=${pieceRow.nfacture}`, 'import-invoice', 'processAndSaveImpayes', { nfacture: pieceRow.nfacture, isNew: isNewImpaye });
        await impaye.save(null, { useMasterKey: true });
        info(`Impayé sauvegardé avec succès (nfacture=${pieceRow.nfacture})`, 'import-invoice', 'processAndSaveImpayes', { nfacture: pieceRow.nfacture, isNew: isNewImpaye });
        ...
        try {
          ...
          if (shouldLogActivity) {
            ...
            activite.set('payeur_nom', getInterlocuteurField(interlocuteurs, 'Payeur', 'nom') || null);
          ...
          error(`Erreur log activite pour ${pieceRow.nfacture}: ${logErr.message}`, 'import-invoice', 'processAndSaveImpayes', { nfacture: pieceRow.nfacture, error: logErr.message });
        }

      } catch (err) {
        error(`Erreur nfacture=${pieceRow.nfacture}: ${err.message}`, 'import-invoice', 'processAndSaveImpayes', { nfacture: pieceRow.nfacture, error: err.message, stack: err.stack?.substring(0, 500) });
        ...
        try {
          ...
          error(`Erreur log activite erreur pour ${pieceRow.nfacture}: ${logErr.message}`, 'import-invoice', 'processAndSaveImpayes', { nfacture: pieceRow.nfacture, error: logErr.message });
        }
      }
    }

    info(`Étape 5 terminée — ${stats.impayes_created} créés, ${stats.impayes_updated} MàJ, ${stats.contacts_created} contacts créés, ${stats.errors.length} erreurs`, 'import-invoice', 'processAndSaveImpayes', { 
    ...
    error(`Erreur Étape 5: ${err.message}`, 'import-invoice', 'processAndSaveImpayes', { error: err.message, stack: err.stack?.substring(0, 500) });
    throw err;
  }
}
