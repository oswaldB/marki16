...
async function updateDynamicOptionsMaster(options = {}) {
  ...
  console.log(`[update-dynamic-options/master] Début du processus de mise à jour`);
  ...
  try {
    console.log('[update-dynamic-options/master] Étape 1/1: Mise à jour des options dynamiques...');
    stats.result = await updateDynamicOptions();
    console.log(`[update-dynamic-options/master] Mise à jour terminée: ${stats.result?.updated ?? 0} configurations mises à jour`);
  } catch (error) {
    console.error('[update-dynamic-options/master] Erreur:', error.message);
    stats.result = { updated: 0, errors: [error.message] };
    stats.errors.push({ step: 1, script: '01-updateDynamicOptions', error: error.message });
  }
  ...
  console.log(`[update-dynamic-options/master] Durée totale: ${(finishedAt - startedAt) / 1000} secondes`);
  writeLog(`SUCCESS: Processus terminé (${finishedAt - startedAt}ms) - ${stats.result?.updated || 0} configurations mises à jour`);

  // Persistance du log d'exécution global
  try {
    if (process.env.NODE_ENV !== 'test') {
      const log = new Parse.Object('UpdateDynamicOptionsMasterLog');
    ...
    console.error('[update-dynamic-options/master] Impossible d\'écrire le log:', logErr.message);
  }
...
if (require.main === module) {
  updateDynamicOptionsMaster()
    .then((stats) => {
      console.log('Processus update-dynamic-options terminé:', JSON.stringify(stats, null, 2));
