...
async function updateDynamicOptions() {
  ...
  try {
    ...
    console.log(`[updateDynamicOptions] ${configs.length} configurations dynamiques trouvées`);
    ...
    for (const config of configs) {
      try {
        ...
        stats.updated++;
        console.log(`[updateDynamicOptions] Configuration ${config.id} mise à jour`);
      } catch (error) {
        console.error(`[updateDynamicOptions] Erreur configuration ${config.id}:`, error.message);
        stats.errors.push({
          configId: config.id,
          error: error.message
        });
    ...
    console.log(`[updateDynamicOptions] Terminé - ${stats.updated} configurations mises à jour, ${stats.errors.length} erreurs`);
    ...
    try {
      if (process.env.NODE_ENV !== 'test') {
        ...
        const log = new Parse.Object('UpdateDynamicOptionsLog');
        ...
        log.set('updated', stats.updated);
        log.set('errors', stats.errors.map(e => JSON.stringify(e)));
        await log.save(null, { useMasterKey: true });
      }
    } catch (logErr) {
      console.error('[updateDynamicOptions] Impossible d\'écrire le log:', logErr.message);
    }
  }
