...
async function verifyPaidInvoices({ trigger = 'cron' } = {}) {
  ...
  console.log(`[verifyPaidInvoices] Trouvé ${unpaidInvoiceIds.length} factures impayées dans Parse`);
    ...
    async function openDatabaseWithRetry(path, maxRetries = 3, retryDelayMs = 60000) {
      ...
      while (retries < maxRetries) {
        try {
          ...
          console.log(`[verifyPaidInvoices] Connexion DB SQLite réussie (${path})`);
          return db;
        } catch (err) {
          retries++;
          console.error(`[verifyPaidInvoices] Erreur DB SQLite (attempt ${retries}/${maxRetries}): ${err.message}`);
          if (err.message.includes('database disk image is malformed') && retries < maxRetries) {
            console.log(`[verifyPaidInvoices] Attente de 1 minute avant retry...`);
      ...
      throw new Error(`[verifyPaidInvoices] Impossible d'ouvrir la DB après ${maxRetries} tentatives`);
    }
    ...
    console.error('[verifyPaidInvoices] Erreur base de données SQLite:', err.message);
