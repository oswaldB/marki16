<template>
  <div class="p-6 space-y-4">
    ...
    <div class="space-y-4">
      ...
      <!-- Vue par payeur - CARDS FINES GROUPÉES -->
      <div v-else-if="activeView === 'payeur'" class="space-y-4">
        <div class="flex justify-end text-sm text-gray-500">
          {{ groupesPayeur.length }} payeurs · {{ impayes.length }} impayés
        </div>

        <div v-for="payeur in groupesPayeur" :key="payeur" class="space-y-2">
          <div class="bg-gray-50 rounded-lg p-4">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-2">
                <h2 class="font-semibold text-lg text-gray-900">{{ payeur || '(Inconnu)' }}</h2>
              ...
              <UButton
                ...
                @click="ouvrirDrawerAssignationForPayeur(payeur)"
              >
                Attribuer séquence
              </UButton>
            </div>
            ...
            <div class="space-y-1">
              <UCard
                v-for="impaye in impayesForPayeur(payeur)"
                :key="impaye.objectId"
                class="text-sm p-3"
              >
      ...
      <!-- Vue par contact - CARDS FINES GROUPÉES -->
      <div v-else-if="activeView === 'contact'" class="space-y-4">
        <div class="flex justify-end text-sm text-gray-500">
          {{ groupesContact.length }} contacts
        </div>

        <div v-for="contact in groupesContact" :key="contact" class="space-y-2">
          <div class="bg-gray-50 rounded-lg p-4">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-2">
              ...
              <UButton
                ...
                color="sky"
                icon="i-heroicons-queue-list"
                @click="ouvrirDrawerAssignationForContact(contact)"
              >
                Attribuer séquence
              </UButton>
            </div>
            ...
            <div class="space-y-1">
              <UCard
                v-for="impaye in impayesForContact(contact)"
                :key="impaye.objectId"
                class="text-sm p-3"
              >
                <div class="flex items-center justify-between mb-2">
                ...
                <div class="flex flex-wrap items-center gap-4">
                  ...
                  <div class="flex items-baseline gap-3">
                    ...
                    <span v-if="impaye.payeur_nom && impaye.payeur_nom !== contact" class="text-xs text-gray-400">
