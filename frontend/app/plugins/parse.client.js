...
export default defineNuxtPlugin((nuxtApp) => {
    ...
    if (process.client) {
        ...
        const expectedServerURL = config.public.parseServerURL;
        ...
        if (Parse.applicationId !== expectedAppId) {
            console.warn(
                "⚠️ WARNING: Parse app-id mismatch detected! Forcing correction...",
            );
            ...
            if (Parse.User.currentAsync) {
                Parse.User.logOut().then(() => {
                    console.log(
                        "✅ Parse session cleared and reinitialized with correct app-id:",
                        Parse.applicationId,
                    );
                });
            } else {
                console.log(
                    "✅ Parse reinitialized with correct app-id:",
                    Parse.applicationId,
                );
            }
        }
