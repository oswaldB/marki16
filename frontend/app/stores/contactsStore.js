...
export const useContactsStore = defineStore("contacts", {
    ...
    actions: {
        async fetchAllContacts(force = false, page = 1, pageSize = 50) {
            // Si on demande la page 1 et qu'on a déjà un cache valide, retourner les données cachees
            if (!force && this.hasCache && page === 1) {
            ...
            try {
                ...
                const countQuery = new $parse.Query("Contact");
                this.totalCount = await countQuery.count();

                // Compter les contacts sans email sur toute la base
                const emailVideQuery = new $parse.Query("Contact");
                emailVideQuery.equalTo("email", "");

                const emailAbsentQuery = new $parse.Query("Contact");
                emailAbsentQuery.doesNotExist("email");

                const sansEmailQuery = $parse.Query.or(
                    emailVideQuery,
                    emailAbsentQuery,
                );
                this.sansEmailTotalCount = await sansEmailQuery.count();

                const contacts = await query.find();

                // Calculer le nombre d'impayés pour chaque contact (optimisé)
                const contactsWithImpayes =
                    await this.addImpayesCountToContacts(contacts);

                // Si c'est la page 1, mettre à jour allContacts et le cache
                if (page === 1) {
        ...
        async fetchPage(page = 1, pageSize = 50) {
            return this.fetchAllContacts(true, page, pageSize);
        },
        ...
        async addImpayesCountToContacts(contacts) {
            ...
            if (impayesStore.allImpayes.length > 0 && impayesStore.hasCache) {
                ...
                return contacts.map((contact) => {
                    ...
                    const count = impayesStore.allImpayes.filter((i) => {
                        ...
                        return (
                            i._parse &&
                            i._parse.get("payeur") &&
                            i._parse.get("payeur").id === contactId
                        );
                    }).length;
                ...
                try {
                    ...
                    console.warn(
                        "Impossible de charger les impayés, utilisation du fallback:",
                        error,
                    );
                    ...
                    return await Promise.all(
                        contacts.map(async (contact) => {
                            ...
                            const count = await impayeQuery
                                .equalTo("payeur", contact)
                                .count();
                            contact.set("nb_impayes", count);
                            return contact;
                        }),
        ...
        async fetchEntitesGroupes() {
            ...
            const groups = await Promise.all(
                entreprises.map(async (entreprise) => {
                    const employes = await entreprise
                        .relation("employes")
                        .query()
                        .ascending("nom")
                        .find();
        ...
        async fetchParticuliersGroupes() {
            ...
            const groups = await Promise.all(
                candidats.map(async (chef) => {
                    const membres = await chef
                        .relation("employes")
                        .query()
                        .ascending("nom")
                        .find();
            ...
            const groupesAvecMembres = groups.filter(
                (g) => g.membres.length > 0,
            );

            // Mémoriser les IDs des membres pour les exclure des particuliers seuls
            const ids = new Set(this.employeIds);
            groupesAvecMembres.forEach((g) =>
                g.membres.forEach((m) => ids.add(m.id)),
            );
            this.employeIds = ids;
        ...
        groupByCompany(contacts) {
            ...
            contacts.forEach((contact) => {
                ...
                if (contact.get("type_personne") === "M") {
                    const entrepriseNom =
                        contact.get("nom") || "Entreprise sans nom";
                ...
                // Si c'est une personne physique avec une entreprise ET que l'entreprise est une personne morale
                    ...
                    if (entreprise && entreprise.get("type_personne") === "M") {
                        const entrepriseNom =
                            entreprise.get("nom") || "Entreprise sans nom";
                        if (!grouped[entrepriseNom]) {
                            grouped[entrepriseNom] = {
                                entreprise: null,
                                employes: [contact],
            ...
            return Object.entries(grouped).map(([nom, data]) => ({
                ...
                contacts: data.entreprise
                    ? [data.entreprise, ...data.employes]
                    : data.employes,
